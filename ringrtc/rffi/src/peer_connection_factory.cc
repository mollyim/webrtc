...
RUSTEXPORT PeerConnectionInterface* Rust_createPeerConnection(
  ...
  switch (proxy_info_borrowed->type) {
    case kRffiProxyTypeNone: proxy_info.type = rtc::PROXY_NONE; break;
    case kRffiProxyTypeHttps: proxy_info.type = rtc::PROXY_HTTPS; break;
    case kRffiProxyTypeSocks5: proxy_info.type = rtc::PROXY_SOCKS5; break;
    default:
      RTC_LOG(LS_ERROR) << "Unknown proxy type: " << proxy_info_borrowed->type;
  }
  proxy_info.address = rtc::SocketAddress(std::string(proxy_info_borrowed->hostname_borrowed),
